/* checkout-form — el "Order Summary" pegado del prompt de Gus (flujo
   Suscripción), verbatim salvo adaptaciones declaradas:
   · sin "use client" (no es Next).
   · `onPlaceOrder`: el original no exponía el click de "Place Order" y aquí
     tiene que soltar el ticket con confeti. DEMO VISUAL: no cobra nada.
   · El wrapper original era una página entera (min-h-screen bg-background);
     dentro del overlay solo pone su columna — el velo y el centrado los da
     suscripcion-overlay. El resto de clases quedó tal cual. */
...
export default function CheckoutForm({ onPlaceOrder }: { onPlaceOrder?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center p-6">
      ...
      <div className="w-full max-w-md mt-4 flex items-center justify-between rounded-xl border px-4 py-3 bg-card shadow-lg">
        ...
        <Button className="px-6" onClick={onPlaceOrder}>Place Order</Button>
      </div>
    </div>
  )
}
